...
def _nan2zero(x):
    '''
    :return: creates a new tensor with the same shape as x.
    If an element in x is NaN, it is replaced with zero,
...
def _nelem(x):
    nelem = tf.reduce_sum(tf.cast(~tf.is_nan(x), tf.float32))   #calculates the number of non-NaN elements
    return tf.cast(tf.where(tf.equal(nelem, 0.), 1., nelem), x.dtype)   #if nelem=0, new tensor=1.0, otherwise the value of nelem.
